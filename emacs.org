...
* Keybindings
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (global-set-key (kbd "C-x r q") 'kill-emacs-and-tangle) ;; r·eally q·uit
  #+END_SRC
...
* Navigation
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package beginend
      :hook ((prog-mode dired-mode text-mode) . beginend-global-mode))
  #+END_SRC
...
** CTRLF
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package ctrlf
       :bind (("M-s C-s" . ctrlf-forward-literal)
              ("M-s C-r" . ctrlf-backward-literal)
              ("M-s C-M-s" . ctrlf-forward-regexp)
              ("M-s C-M-r" . ctrlf-backward-regexp))
       :config (ctrlf-mode -1)
...
*** Scratch
    #+BEGIN_SRC emacs-lisp
      (use-package temporary-persistent
        :defer t
        :config (setq temporary-persistent-buffer-name-template "temp")
        (setq temporary-persistent-store-folder (expand-file-name "temp" base-emacs-directory)))
...
** Magit
   #+BEGIN_SRC emacs-lisp
     (use-package magit
       ...
       :custom-face (magit-branch-current ((t (:foreground "#5cEfFF" :box (:line-width (3 . 3) :color "#5cEfFF" :style released-button)))))
       :config
...
** Docker
   #+BEGIN_SRC emacs-lisp
     (use-package docker :disabled t)
     (use-package dockerfile-mode :disabled t)
   #+END_SRC
...
* Mail
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package mu4e
      ...
      :ensure nil
      :commands mu4e
      :custom-face (mu4e-flagged-face ((t (:foreground "gold"))))
      ...
      ;; bookmarks  ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
      (setq mu4e-bookmarks
            '((:name "Nouveaux"         :query (concat "date:3d.."
                                                       " AND %unread"
                                                       " AND NOT %hidden")      :key ?n)
              (:name "Non lus"          :query (concat "%unread"
                                                       " AND NOT %hidden")      :key ?u)
              (:name "Perso"            :query (concat " %unread"
                                                       " AND NOT %work"
                                                       " AND NOT %hidden")      :key ?p)
              (:name "Enercoop"         :query (concat " date:24h.."
                                                       " AND %work"
                                                       " AND NOT %hidden")      :key ?e)
              (:name "Importants"       :query "flag:flagged"                   :key ?i)
              (:name "Récents"          :query "date:5h.."                      :key ?r)
              (:name "Aujourd’hui"      :query "date:today.."                   :key ?t)
              (:name "Hier"             :query "date:1d.."                      :key ?h)
              (:name "Dernière semaine" :query "date:7d.." :hide-unread t       :key ?w)
              (:name "Dernier mois" :query "date:1m.."     :hide-unread t       :key ?m)
    ...
    (use-package mu4e-query-fragments
      ...
      (setq mu4e-query-fragments-list
            '(("%unread"  . "flag:unread")
              ...
              ("%gmail"   . "maildir:/Gmail")
              ("%work"    . "maildir:/ProtonMail/Folders/.Enercoop-forward OR maildir:/ProtonMail/Labels/.Enercoop"))
    ...
    (use-package mu4e-alert
      ...
      (setq
       ...
       ;; mu4e-alert-email-count-title "Nouveaux mails !"
...
* Web browsing
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package eww
      :defer t
      :config
      (setq browse-url-generic-program "eww" ;; set default browser to eww
...
* Mode-line
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package doom-modeline
      ...
      ;; I don’t want to see the battery status when it is fully charged ;;;;;;;;;;
      (defun battery-full-p ()
        "Return t if battery is full."
        (interactive)
        (string-equal "100%%" (prin1-to-string (cdr doom-modeline--battery-status) t)))
