...
* Org-mode
  #+BEGIN_SRC emacs-lisp
    (use-package org
      ...
      :custom-face
      (org-agenda-date ((t (:foreground "#a991f1" :inverse-video nil :box (:line-width (2 . 2) :color "grey75" :style released-button) :weight ultra-bold))))
      (org-agenda-date-today ((t (:foreground "#a991f1" :inverse-video t :box (:line-width (2 . 2) :color "grey75" :style released-button) :weight ultra-bold))))
      (org-agenda-date-weekend ((t (:foreground "#68228b" :inverse-video nil :box (:line-width (1 . 1) :color "grey75" :style released-button) :weight ultra-bold))))
      ...
      (setq org-agenda-include-diary t
            ...
            org-agenda-start-day "2021-07-06"
      ...
      (setq org-todo-keyword-faces
            '(("TODO"       . (:foreground "pink"      :weight bold))
              ...
              ("BLOQUÉ"     . (:foreground "pink4"     :weight bold))
      ...
      (setq super-groups-main
            '(;; Each group has an implicit boolean OR operator between its selectors.
              ...
              (:name "PLANNING"
                     ...
                     :transformer
                     (--> it
                       (cond ((string-match-p "DONE" it) it)
                             ((string-match-p "WIP\\|STARTED\\|DELEGATED" it)
                              (propertize it 'face '(:foreground "yellow")))
                             ((string-match-p "\\[#A\\]" it)
                              (propertize it 'face '(:foreground "red" :weight bold)))
                             ((string-match-p "\\[#C\\]\\|\\[#D\\]\\|\\[#E\\]" it)
                              (propertize it 'face '(:foreground "#696969")))
                             ((string-match-p "WAIT\\|BLOQUÉ\\|DEFERRED\\|BLOCKED" it)
                              (propertize it 'face '(:foreground "#8b636c")))
                             ((string-match-p "WORK" it)
                              (propertize it 'face '(:foreground "#00cd00")))
                             (t it)))
                     :discard (:todo "DONE"))
