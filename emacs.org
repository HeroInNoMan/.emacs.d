...
*** Helm package
    #+BEGIN_SRC emacs-lisp
      (use-package helm
        ...
        (defhydra hydra-helm-config(:color teal :hint nil)
          "
          ^Helm^    ^Find^         ^Search^      ^Navigate^       ^Complete^      ^Doc & Help^    ^Other^
          ^────^────^────^─────────^──────^──────^────────^───────^───^───────────^──────────^────^─────^────────────
          _r_esume  _l_ locate     _R_ regexp    _e_ etags        _<tab>_  lisp   _h e_ eldoc     _t_ top
          _q_uit    _/_ find       _o_ occur     _i_ imenu        _$_     lines   _h i_ info      _p_ process
          ^ ^       _F_ files      _a_ ag grep   _I_ imenu all    _g_    google   _h r_ emacs     _c_ colors
          ^ ^       _f_ all files  ^ ^           _._ imenu any    _s_   surfraw   _h g_ gnus      _8_ ucs
          ^ ^       _m_ mini       ^ ^           _b_ bookmark     _w_ wikipédia   _h h_ doc       _,_ calcul
          ^ ^       ^ ^            ^ ^           _'_ register     ^ ^             _h a_ apropos   _P_ packages
          ^ ^       ^ ^            ^ ^           _y_ kill-ring    ^ ^             _h m_ woman     _X_ xfont
          ^ ^       ^ ^            ^ ^           _<SPC>_ mark ring^ ^             ^ ^             _x_ run external
          "
          ;; find files & buffers
          ("l"     helm-locate)
          ("/"     helm-find)
          ("F"     helm-find-files)
          ("f"     helm-for-files) ;; like helm-multi-files but always run locate
          ("m"     helm-mini)
          ;; search
          ("R"     helm-regexp)
          ("o"     helm-occur)
          ...
          ;; documentation & help
          ("h e"   helm-eval-expression-with-eldoc)
...
*** meghanada
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package meghanada
        ...
        (before-save-hook . meghanada-code-beautify-before-save)
        :config
