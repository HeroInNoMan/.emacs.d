...
* Startup
  ...
  #+BEGIN_SRC emacs-lisp
    ;;; emacs.el --- user-init-file                    -*- lexical-binding: t -*-
  #+END_SRC
  ...
  Suppress warnings about lexical binding. Some packages store their data in Elisp files without lexical bindings or simply don’t use it yet.
  #+BEGIN_SRC emacs-lisp
    (setopt warning-suppress-types '((lexical-binding)))
  #+END_SRC

  “Disable” garbage collection while starting up:
  #+BEGIN_SRC emacs-lisp
...
* Custom functions
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package my-functions ;; custom functions
      ...
      ("M-é"             . previous-buffer)          ;; call previous buffer
      ("M-è"             . next-buffer)              ;; call next buffer
      ("C-M-é"           . previous-user-buffer)     ;; call previous user buffer
      ("C-M-è"           . next-user-buffer)         ;; call next user buffer
...
** Peek
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package peek
       ...
       :custom-face (peek-overlay-content-face
                     ((t (:background "grey10"))))
...
* Dired
  #+BEGIN_SRC emacs-lisp
    (use-package dired
      ...
      (define-advice dired-mark-read-file-name (:around (func &rest args) ask-to-make-dir)
...
* Navigation
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    (use-package dumber-jump
      :init (add-hook 'xref-backend-functions #'dumber-jump-xref-activate)
      :bind (:map prog-mode-map
                  ("C-." . xref-find-definitions)
                  ("C-," . xref-go-back))
      ;; :bind (:map prog-mode-map
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package dogears
      :init (dogears-mode)
      :config
      (pretty-hydra-define dogears-hydra
        (:color blue :quit-key "q")
        ("Go" (("g" dogears-go "→")
               ("p" dogears-back "↶" :color red)
               ("n" dogears-forward "↷" :color red))
         "List" (("l" dogears-list "list…")
                 ("s" dogears-sidebar "sidebar"))))
      :bind
      ("<f6>" . dogears-hydra/body)
      ("M-g d" . dogears-go)
...
** visual-replace
   #+BEGIN_SRC emacs-lisp
     (use-package visual-replace
       :straight '(visual-replace :host github :repo "szermatt/visual-replace")
       :bind (("C-%" . visual-replace)
...
*** [[https://github.com/minad/consult][Consult]]
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package consult
        ...
        (pretty-hydra-define hydra-consult-config
          ...
          ("Navigate"
           ...
           "Search"
           (("A"     consult-org-agenda    "org-agenda")
            ("l"     consult-locate        "locate")
            ("/"     consult-fd            "find")
            ("g"     consult-git-grep      "git grep")
            ("\\"    consult-git-log-grep  "git log")
            ("x"     deadgrep              "deadgrep")
           ...
           (("B"     describe-bindings     "bindings")
            ...
            ("n"     info-display-manual   "info page")
            ("h"     consult-info          "info full text")
            ("w"     woman                 "(wo)man")
            ("m"     consult-man           "man -k"))
           "Session"
      ...
      (use-package consult-gh
        :if (= 0 (shell-command "hash gh")) ;; check if gh is present on the system
        ...
        (add-to-list 'consult-gh-default-orgs-list "arthur.leothaud@enercoop.org")
        ...
        (setq consult-gh-default-clone-directory my-default-project-root))
...
*** [[https://github.com/minad/orderless][Orderless]]
    ...
    #+BEGIN_SRC emacs-lisp
      (use-package orderless
        ...
        (defun toggle-fuzzy-matching ()
          "Toggle fuzzy matching of components."
          (interactive)
          (if (and (boundp 'ale-fuzzy-matching-p)
                   ale-fuzzy-matching-p)
              (progn (setq ale-fuzzy-matching-p nil)
                     (setq orderless-matching-styles '(orderless-literal)))
            (progn (setq ale-fuzzy-matching-p t)
...
** Buffer Management
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (key-chord-define-global (kbd "+-") 'ale/kill-buffer-and-maybe-window) ;; kill buffer without confirmation
   #+END_SRC
...
** Magit
   #+BEGIN_SRC emacs-lisp
     (use-package magit
       ...
       (git-commit-setup . smartparens-mode)
...
* Date
  ...
  #+BEGIN_SRC emacs-lisp
    ...
    ;; (transient-define-prefix my/transient-dates ()
    ;;   "Dates"
...
** Time
   ...
   [[https://github.com/protesilaos/tmr][TMR]] is a nice package that enables timers that popup at a desired time.

   #+BEGIN_SRC emacs-lisp
     (use-package tmr :disabled)
   #+END_SRC
               ...
               ((agenda "" ((org-agenda-overriding-header "📆 Agenda")
                ...
                (tags-todo "enercoop" ;; todo
                           ((org-agenda-overriding-header "☑️ À faire")
                            ...
                            (org-agenda-skip-function '(org-agenda-skip-entry-if
                                                        ...
                                                        'scheduled
                                                        'regexp "#A"
                                                        'todo '("STARTED" "WAIT" "BLOCKED" "MAYBE")))))
                (tags-todo "enercoop" ;; blocked
                           ((org-agenda-overriding-header "🚧 Bloqué")
                            (org-agenda-sorting-strategy '(todo-state-up
                                                           priority-down
                                                           user-defined-down
                                                           effort-up
                ...
                (tags-todo "enercoop" ;; waiting
                           ((org-agenda-overriding-header "⌚ En attente")
                            ...
                            (org-agenda-skip-function '(org-agenda-skip-entry-if
                                                        'nottodo '("WAIT")))))
                (tags-todo "enercoop" ;; maybe
                           ((org-agenda-overriding-header "🤷 Un jour peut-être")
                            (org-agenda-sorting-strategy '(todo-state-up
                                                           priority-down
                                                           user-defined-down
                                                           effort-up
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package smartparens
       ...
       (pretty-hydra-define hydra-parens
         (:color teal :quit-key "q")
         ("Move"
          (("<right>" sp-forward-sexp "SEXP ⟶")
           ("<left>" sp-backward-sexp "SEXP ⟵")
           )
          "Insert"
...
*** lsp-java
    #+BEGIN_SRC emacs-lisp
      ...
      (use-package lsp-java
        ...
        :hook (java-mode . (java-mode-config lsp))
        :bind ("C-S-o" . lsp-execute-code-action)
        :init (defun java-mode-config ()
        ...
        ;; Enable dap-java
        (require 'dap-java)
        (setq lsp-java-java-path "~/.sdkman/candidates/java/current")
...
** Web
   ...
   #+BEGIN_SRC emacs-lisp
     (use-package impatient-showdown
       :config (setq impatient-showdown-flavor 'github))
   #+END_SRC
   ...
   #+BEGIN_SRC emacs-lisp
     (pretty-hydra-define hydra-toggles
       ...
       ("Navigation"
        ...
        "Highlighting"
        (
         ("C" corfu-automatic                            "auto-completion"      :toggle corfu-auto)
        ...
        (("(" smartparens-mode                          "smartparens"          :toggle t)
         ...
         ("z" toggle-fuzzy-matching                     "fuzzy matching"       :toggle ale-fuzzy-matching-p)
     ...
     (pretty-hydra-define hydra-widgets
       ...
       ("Shells"
        ...
        (;; ("(" mu4e "mu4e")
         ...
         ("k" docker "docker")
         ("K" docker-compose "docker-compose"))
        "Diff"
        (("d" ediff "ediff")
...
** Emacs from outside
   ...
   Thanks to [[https://protesilaos.com/codelog/2024-09-19-emacs-command-popup-frame-emacsclient/][this]] post I improved the way I summon emacs popup frames from outside of emacs.
   #+BEGIN_SRC emacs-lisp
     ...
     (use-package keycast
       ...
       (with-eval-after-load 'keycast
         (define-minor-mode keycast-mode
           "Show current command and its key binding in the mode line, adapted."
           :global t
           (if keycast-mode
