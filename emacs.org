      ...
      or jump to first active channel, or cycle through all channels."
       ...
       (defun next-erc-channel-buffer ()
         ...
         (let ((curr-buff (current-buffer))
           ...
           (while (and (not found-p)
             ...
             (if (erc-channel-buffer-p)
                 (setq found-p t)
               (next-buffer)))))

       (defun erc-channel-buffer-p ()
         "Return t if `current-buffer' is an ERC channel, nil otherwise."
         (interactive)
         (and (string-prefix-p "#" (buffer-name))
              (eq major-mode 'erc-mode)))

       (defun erc-quote-last-message (n)
         "Cite the last Nth message in current ERC channel."
         (interactive "p")
         (if (erc-channel-buffer-p)
           ...
           (message "You are not in an ERC channel.")))
       ...
       (eval-after-load 'erc-track
         '(progn
            ...
            (overlay-put erc-bar-overlay 'face '(:background "black" :overline "red"))
