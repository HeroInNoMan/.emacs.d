...
* Libraries & tools
  ...
  #+BEGIN_SRC emacs-lisp
    (use-package my-functions ;; custom functions
      :ensure nil
      :commands (ale-insert-ticket-prefix ale-jirify ale-find-init-file ale-open-project ale/apply-local-theme)
      :load-path "elisp/"
      :bind
   ...
   #+BEGIN_SRC emacs-lisp
     (setq display-time-day-and-date t ;; display date and time
           ...
           display-time-string-forms '((if (and (not display-time-format) display-time-day-and-date)
                                           (format-time-string "%H:%M, %a %d %b (w%V) " now))))
...
** Dealing with parens
   ...
   #+BEGIN_SRC emacs-lisp
     ...
     (defhydra hydra-parens(:color teal :hint nil)
       "
       ^Parens^     ^Edit^                               ^Move^                ^Slurp & Barf^
       ^──────^─────^────^───────────────────────────────^────^────────────────^────────────^─────────
       _q_ quit     _C-j_ newline                        _u_ backward-up-sexp  _(_ backward-slurp-sexp
