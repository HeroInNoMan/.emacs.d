...
(defcustom define-word-services
  '((le-dictionnaire "https://www.le-dictionnaire.com/definition/%s" define-word--parse-le-dictionnaire)
...
(defcustom define-word-default-service 'le-dictionnaire
  "The default service for define-word commands. Must be one of `define-word-services'."
  :type '(choice
          (const le-dictionnaire)
          (const wordnik)
          (const openthesaurus)
          (const webster)
...
(defun define-word--parse-le-dictionnaire ()
  "Parse output from le-dictionnaire site and return formatted list."
  (save-match-data
    (let (results beg part)
      (while (re-search-forward "<div class=\"defbox\">[^(]*(\\([^)]+\\))" nil t)
        (setq part (match-string 1))
        (unless (= 0 (length part))
          (setq part (concat part " ")))
        (setq beg (search-forward "<li>"))
